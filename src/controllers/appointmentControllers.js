...
const getAppointmentsForPatient = async (req, res, next) => {
    try {
        ...
        const appointments = await AppointmentModel.find(query.criteria, query.options.fields)
            .populate([
                { path: "patient", select: "_id name surname image" },
                { path: "doctor", select: "_id name surname image" },
                { path: "clinic", select: "_id name  image" },
        ...
        if (appointments.length > 0) {
            res.status(200).send({
                links: query.links(`/patientAppointments/${req.params.filterAppointments}`, total),
                appointments,
            })
        } else {
            const err = new Error()
    ...
    try {
        ...
        if (req.params.filterAppointments === "none") {
            ...
            if (req.user._id.toString() !== req.params.userId.toString()) {
                ...
                appointments = await AppointmentModel.find(query.criteria, query.options.fields)
                    .populate([
                        { path: "patient", select: "_id name surname image" },
                        { path: "doctor", select: "_id name surname image" },
                        { path: "clinic", select: "_id name  image" },
...
const addAppointment = async (req, res, next) => {
    try {
        ...
        const clinic = req.body.clinic ? await UserModel.findById(req.body.clinic) : null
...
const removeAppointment = async (req, res, next) => {
    try {
        const deletedAppointment = await AppointmentModel.findByIdAndDelete(req.params.appointmentId)
